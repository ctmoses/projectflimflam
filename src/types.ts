...
export interface IClass {
    ...
    calcrecoveryroll(con:number, level:number, feats?: IFeats[]): number[],  //SM: Not sure what we want to return here...just something like 4d8+4?
    calcmeleehit(attr:number, level:number, feats?: IFeats[]):number,
    calcrangedhit(attr:number, level:number, feats?: IFeats[]):number,
    calcmeleedmg(attr:number, level:number, feats?: IFeats[]):number[],
    calcrangeddmg(attr:number, level:number, feats?: IFeats[]):number[], 
